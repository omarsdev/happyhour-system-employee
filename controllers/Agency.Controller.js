...
exports.createSubAgency = asyncHandler(async (req, res, next) => {
  ...
  const agency = await queryParamsArrayConnection(
    `INSERT INTO agency(city_id, manager_id, parent_agency_id, name_en, name_ar, isHQ) VALUES(?, ?, ?, ?, ?, ?)`,
    [city_id, manager_id, req.params.parent_id, name_en, name_ar, false]
  ).then((result) => {
    res.status(200).json({
      success: true,
      data: result,
