...
exports.createEmployee = asyncHandler(async (req, res, next) => {
  ...
  var id;
  const emp_id = await queryParamsConnection(
...
exports.Login = asyncHandler(async (req, res, next) => {
  ...
  var id;
  const emp_id = await queryParamsConnection(
    "SELECT id, isTerminated FROM employee WHERE email = ?",
    [email]
  ).then((result) => {
...
exports.UpdateEmployeeById = asyncHandler(async (req, res, next) => {
  ...
  /* if (marital_status == 0) {
    ...
    // TODO PROCEDURE FOR EMAIL CHECK
...
exports.Terminate = asyncHandler(async (req, res, next) => {
  const UpdateEmployeeById = await queryParamsArrayConnection(
    "UPDATE employee SET isTerminated = ? WHERE id = ?",
    [1, req.params.id]
  ).then((result) => {
    res.status(200).json({
      success: true,
