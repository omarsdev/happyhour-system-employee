...
exports.getAllReportsBySenderID = asyncHandler(async (req, res, next) => {
  const allReports = await queryConnection(
    "SELECT * FROM report WHERE sender_id = ?",
    [req.params.id]
  ).then((result) => {
    res.status(200).json({
      success: true,
...
exports.getAllReportsByReceiverID = asyncHandler(async (req, res, next) => {
  const allReports = await queryParamsConnection(
    "SELECT * FROM report WHERE receiver_id = ?",
    [req.params.id]
  ).then((result) => {
    res.status(200).json({
      success: true,
